...
import { Bell, CheckCheck, Loader2, RefreshCw, BellOff, Trash2, ArrowRight } from 'lucide-react';
...
function NotifItem({ 
  ...
  return (
    ...
    >
      ...
      {/* Navigate arrow (always visible on hover) + Delete button */}
      <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-1">
        <span
          className={`p-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-all duration-200 ${isDark ? 'text-indigo-400' : 'text-blue-500'}`}
        >
          <ArrowRight size={14} />
        </span>
        <button
          ...
          className={`p-2 rounded-xl opacity-0 group-hover:opacity-100 transition-all duration-200 ${isDark ? 'text-slate-500 hover:text-rose-400 hover:bg-rose-950/30' : 'text-slate-400 hover:text-rose-500 hover:bg-rose-50'}`}
          title="Xóa thông báo"
        >
          <Trash2 size={16} />
        </button>
...
export default function ShopNotifications() {
  ...
  const handleNavigate = (notif: AppNotification) => {
    const route = getNotificationRoute(notif, 'shop');
  ...
  return (
    <div className="w-full px-6 md:px-10 py-8 max-w-5xl mx-auto animate-in fade-in duration-500">
      {/* Header */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-8">
        <div>
          <h1 className={`text-3xl font-black tracking-tight flex items-center gap-3 ${isDark ? 'text-white' : 'text-slate-900'}`}>
            <Bell className={`w-8 h-8 ${isDark ? 'text-indigo-400' : 'text-blue-600'}`} />
            Thông báo
            {unreadCount > 0 && (
              <span className={`text-xs font-black px-2.5 py-0.5 rounded-full ${isDark ? 'bg-indigo-600/30 text-indigo-400 border border-indigo-500/30' : 'bg-blue-600 text-white'}`}>
                {unreadCount}
              </span>
            )}
          </h1>
        ...
        <div className="flex items-center gap-3 shrink-0">
          <button
            onClick={() => refetch()}
            className={`p-2.5 rounded-xl border transition-all ${isDark ? 'border-slate-700 text-slate-450 hover:bg-slate-800 hover:text-white' : 'border-slate-200 text-slate-500 hover:bg-slate-50'}`}
            title="Làm mới"
          >
          ...
          {unreadCount > 0 && (
            <button
              onClick={() => markAllRead()}
              className={`flex items-center gap-2 px-4 py-2.5 text-white rounded-xl font-bold text-xs transition-all hover:opacity-90 shadow-lg ${isDark ? 'bg-indigo-600 shadow-indigo-500/20' : 'bg-[#1a2b4c] shadow-indigo-500/10'}`}
            >
          ...
          {notifications.some(n => n.isRead) && (
            <button
              onClick={() => deleteRead()}
              className={`flex items-center gap-2 px-4 py-2.5 text-white rounded-xl font-bold text-xs transition-all hover:opacity-90 shadow-lg ${isDark ? 'bg-rose-600/90 shadow-rose-600/20' : 'bg-rose-600 shadow-rose-600/20'}`}
            >
      ...
      {/* Main Container Card */}
      <div className={`p-6 md:p-8 space-y-6 rounded-3xl border ${isDark ? 'admin-glass-card bg-slate-900/40 border-white/10' : 'bg-white border-slate-100 shadow-sm'}`}>
        {/* Filter tabs */}
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className={`text-sm mt-1 max-w-sm ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
              {filter === 'unread' 
                ...
                : 'Các thông báo từ hệ thống và đơn đặt lịch sẽ được cập nhật tại đây.'}
            </p>
          </div>
        ) : (
            ...
            {grouped.map(group => (
              <div key={group.label}>
                ...
                <div className="flex flex-col gap-2">
                  {group.items.map(n => (
                    <NotifItem key={n.id} notif={n} onRead={markRead} onDelete={deleteSingle} isDark={isDark} onNavigate={handleNavigate} />
                  ))}
                </div>
              </div>
            ))}
